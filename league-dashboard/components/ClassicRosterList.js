...
export default function ClassicRosterList({ roster, hasLineupOrder, currentGw }) {
  const starters = hasLineupOrder ? roster.filter((p) => p.positionSlot <= 11) : roster;
    ...
    <>
      ...
      {!hasLineupOrder && (
        <p className="border-t border-pitch-border px-4 py-3 text-xs text-ink-dim">
          GW{currentGw} picks aren&rsquo;t available for this team yet.
        </p>
      )}
    </>
  );
